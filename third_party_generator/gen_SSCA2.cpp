...
//    fprintf(stderr, "done\n");
    ...
    free(weights);
	g->m = 2*numEdges;
    g->weights = (weight_t *) malloc(2*numEdges * sizeof(weight_t));
    g->rowsIndices = (edge_id_t *) malloc((TotVertices+1)*sizeof(edge_id_t));
    g->endV = (vertex_id_t *) malloc(2*numEdges * sizeof(vertex_id_t));
    assert(g->rowsIndices != NULL);
    assert(g->endV != NULL);
    g->rowsIndices[0] = 0;
    int l = 0;
    for(uint32_t i = 0; i < TotVertices; ++i) { 
        for (uint32_t j = 0; j < dests[i].size(); j++) { 
            g->endV[l] = dests[i][j];
            g->weights[l] = weight_vect[i][j];
            l++;
        }
        g->rowsIndices[i+1]=l;
    }
